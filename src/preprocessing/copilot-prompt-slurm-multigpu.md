# GitHub Copilot Prompt — Multi-GPU embedding with SLURM on BIU cluster

## Context

The CDSS-ML preprocessing pipeline runs on the BIU SLURM cluster (L4-12h partition,
NVIDIA L4 GPUs, sm_89). The embedding step (`embed_features.py`) currently runs
sequentially on a single GPU and is the slowest part of the pipeline. The L4-12h
partition allows up to 2 GPUs per job. This prompt implements:

1. `embed_features.py` — multi-GPU inference via `torch.multiprocessing`, 2 workers,
   one per GPU, each handling half the 18 features concurrently. Includes all performance
   optimisations (per-feature max_length caps, dynamic padding, dynamic batch size,
   non-blocking GPU transfer, single GPU→CPU transfer, pre-grouped labs) and resume logic
   (per-feature output validity check, atomic writes).
2. `run_pipeline.py` — `--skip-modules` CLI flag so the main pipeline skips
   `embed_features` when embedding is run as a separate SLURM job.
3. Three SLURM job scripts: `pipeline_job.sh`, `embed_job.sh`, `combine_job.sh`
4. `submit_all.sh` — one-command submission that chains all three jobs with
   `--dependency`.

All files live in the root of the `HRS/` repository unless otherwise specified.

---

## File 1 — `embed_features.py` (full rewrite of `run()` and supporting functions)

### 1a. New constants — add after `_TEXT_FEATURES`

```python
# Per-feature maximum token length. Overrides BERT_MAX_LENGTH for short features.
# Prevents padding chief complaint (5 words) to 8192 tokens.
_MAX_LENGTH_CAP: dict[str, int] = {
    "diag_history_text":      512,
    "chief_complaint_text":   64,
    "triage_text":            256,
    "discharge_history_text": 4096,
    "radiology_text":         1024,
    # lab groups: capped at 2048, applied in _worker
}
_LAB_MAX_LENGTH = 2048
_REFERENCE_LENGTH = 512   # batch size is calibrated for this sequence length
```

### 1b. Updated `_get_device` — log SLURM context and GPU details
...
### 1c. Updated `_embed_texts` — all performance optimisations applied
...
def _embed_texts(
    ...
    """
    Return a (N, hidden_size) float32 array using mean pooling over content tokens.

    Optimisations applied:
    - padding="longest": pads only to longest sequence in each batch (not max_length)
    - non_blocking=True: async CPU→GPU transfer
    - Embeddings accumulated on GPU, single .cpu() call after all batches
    - Empty text → zero vector (vectorised assignment)
    """
    import torch  # type: ignore
    ...
    for start in tqdm(
        ...
        batch_texts_safe = [
            t if isinstance(t, str) and t.strip() else " " for t in batch_texts
        ]
        ...
        encoded = tokenizer(
            ...
            truncation=True,
            max_length=max_length,  # hard ceiling
            return_tensors="pt",
        )
        ...
        mean_embeddings = sum_embeddings / sum_mask            # stays on GPU
...
### 1d. New helper — `_effective_batch_size`

```python
def _effective_batch_size(base_batch_size: int, effective_max_length: int) -> int:
    """
    Scale batch size inversely with sequence length to maintain constant token budget.
    ...
    return max(1, min(scaled, base_batch_size * 8))
```

### 1e. New helper — `_output_is_valid`

```python
def _output_is_valid(path: str, expected_rows: int, embedding_col: str) -> bool:
    """
    Return True if a completed embedding parquet exists and passes all validity checks:
    - file exists and is readable
    - correct row count
    - embedding column present
    - no null values in embedding column
    """
...
### 1f. New function — `_build_feature_tasks`
...
def _build_feature_tasks(
    ...
    if labs_df is not None and lab_panel_config:
        logger.info("Pre-grouping labs by panel (one scan for all 13 groups)…")
        itemid_to_group = {
            itemid: gname
...
### 1h. Rewrite `run()` — add `--config` CLI arg and multi-GPU dispatch

Replace the entire `run()` function and add a `main()` entry point:
...
## File 2 — `run_pipeline.py` — add `--skip-modules` flag

In `run_pipeline.py`, find the `argparse` section in `main()` and add:
...
Then in the execution loop, add a skip check:

```python
for idx, module_name in enumerate(modules_to_run, start=1):
    if module_name in (args.skip_modules or []):
        logger.info(
...
## File 3 — `embed_job.sh` (create in HRS/ root)
...
## File 4 — `pipeline_job.sh` (create in HRS/ root)
...
## File 5 — `combine_job.sh` (create in HRS/ root)
...
## File 6 — `submit_all.sh` (create in HRS/ root)
...
# After interruption — resume embedding + combine only:
bash submit_all.sh --embed-only

# Monitor:
squeue -u $USER

# Check embedding log:
tail -f logs/hrs_embed_<JOBID>.out
```

---

## Config additions to `preprocessing.yaml`

```yaml
# Multi-GPU embedding settings
# BERT_DEVICE: cuda — use GPU(s). Set to cpu to force CPU mode.
BERT_DEVICE: cuda
